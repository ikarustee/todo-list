...
function updateScreen() {
    ...
    todoArray.forEach(el => {
        ...
        ${el.id == editMode ? `<span id="${el.id}" class="todotext todotext-${el.id} edit" contenteditable> ${el.text}</span>` : `<span id="${el.id}" class="todotext todotext-${el.id} ${el.todoStatus ? "todo--done" : ""}"> ${el.text}<br><span class="todo--date ${el.dueDate < date ? 'due' : ''}">${el.dueDate}</span></span>`}
